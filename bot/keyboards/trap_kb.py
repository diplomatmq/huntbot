...
def get_trap_status_keyboard(user_id: int):
    """Keyboard for trap status with skip button (callback for invoice generation)"""
    buttons = [
...
def get_trap_setup_keyboard(invoice_link: str):
    """Keyboard for trap setup with direct invoice URL button"""
    buttons = [
        [InlineKeyboardButton(
            text="⏭️ Пропустить кд (5 ⭐)",
            url=invoice_link
